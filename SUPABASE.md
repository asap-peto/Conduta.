...
| Ao criar/entrar em liga: `null value in column "league_id" … violates not-null constraint` | tabela `league_members` de um **modelo antigo** (usava `league_id NOT NULL`) que é incompatível com o atual (`group_code`) | **caminho garantido** (dados de liga são só de teste): rode `DROP TABLE IF EXISTS league_members CASCADE; DROP TABLE IF EXISTS league_groups CASCADE;` e depois o `schema.sql` inteiro. O `schema.sql` atual também tenta auto-curar com `DROP COLUMN IF EXISTS league_id`, mas o drop das tabelas é o mais confiável |
