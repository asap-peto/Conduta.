...
function getTodayCompletions() {
  ...
  return player.levelsCompleted.filter(c => c.completedAt && c.completedAt.slice(0, 10) === today);
}
...
function buildMascotMessage() {
  ...
  return `Paciente esperando! Nível ${player.currentLevel} está pronto.`;
}
...
function renderTodayCard() {
  ...
  if (!today && !selectedLevel) {
    ...
    card.innerHTML = `
      <div class="today-top">
        <div class="today-art">${icon('trophy', 44)}</div>
  ...
  card.innerHTML = `
    <div class="today-top">
      <div class="today-art mode-${selectedLevel.mode}">${icon(mode.icon, 44)}</div>
      <div class="today-head">
...
function renderLevelMap() {
  ...
  const visible = LEVELS
    .filter(lv => lv.number <= player.currentLevel)
...
function renderLevelNode(lv, idx) {
  ...
  const isCompleted = lv.number < player.currentLevel;
  const isCurrent   = lv.number === player.currentLevel;
  const isLocked    = lv.number > player.currentLevel;
...
function toggleTodayCard(levelId) {
  ...
  if (!level || level.number > player.currentLevel) return;
...
function openPastLevel(levelId) {
  ...
  body.innerHTML = `
    <div class="past-head">
      <div class="past-art mode-${lv.mode}">${icon(mode.icon, 36)}</div>
      <div class="past-info">
...
function renderQuests() {
  ...
  const today = todayKey();
  const levelsToday = player.levelsCompleted.filter(c => c.completedAt && c.completedAt.slice(0,10) === today).length;
  const perfectToday = player.levelsCompleted.filter(c => c.completedAt && c.completedAt.slice(0,10) === today && c.perfect).length;
