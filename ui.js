...
function renderCase() {
  ...
  for (var i = 1; i <= Math.min(gs.attempts, MAX - 1); i++) {
    ...
    b.innerHTML =
      '<div class="clue-label">' + (c.clue_labels[i] || 'Pista ' + (i + 1)) + '</div>' +
...
function renderArchiveList() {
  ...
  var diffLabels = { 1: 'Fácil', 2: 'Médio', 3: 'Difícil' };
  var diffClass  = { 1: 'easy',  2: 'medium', 3: 'hard' };
  ...
  for (var d = todayGlobal - 1; d >= 0; d--) {
    ...
    try { saved = JSON.parse(localStorage.getItem('conduta_game_' + key)); } catch(e) {}
